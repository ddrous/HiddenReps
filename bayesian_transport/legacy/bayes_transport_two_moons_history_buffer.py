...
Three identically initialized transports share exactly the same simulator pairs, acquisition
weights, minibatch order, particle counts, and number of updates:
    single_observation: exact-prior inputs only (matched amortized SBI baseline);
    no_replay: original interpolation, but no historical posterior-cloud replay;
    buffered: original interpolation + nearest-observation historical posterior-cloud replay.
All three replay the SAME finite simulation DATASET. This is distinct from posterior-cloud replay.
The no_replay ablation isolates the latter; single_observation is not an external NPE/SNPE model.
Original truth-anchored interpolation is retained explicitly, including its potential training
shortcut. Neither held-out theta nor diagnostic likelihoods enter learned inference.
...
Evaluation transitions are configurable: brownian, driftBrownian, constantVelocity, rotation,
ou, custom, and relocation, plus optional stable/independent controls. Boundaries reflect into
...
class Config:
    ...
    categorical_proposal_enabled: bool = False  # Broad prior training for held-out sequences.
    ...
    # Current-only baseline evaluation starts uniform; sequential methods carry incoming clouds.
    ...
    evaluation_transitions: tuple[str, ...] = (
        "brownian", "driftBrownian", "constantVelocity", "rotation", "ou", "custom", "relocation",
    )
    evaluation_controls: bool = True  # Also include stable and independent negative controls.
...
_allowed_transitions = {"brownian", "driftBrownian", "constantVelocity", "rotation", "ou", "custom", "relocation"}
...
if CFG.evaluation_controls:
    SCENARIOS += (Scenario("stable"), Scenario("independent", reset_probability=1.0))
...
def predict_grid_mass(mass: np.ndarray, axis: np.ndarray, time_index: int,
    ...
    if scenario.name in ("stable", "brownian", "relocation", "independent"):
...
#%% 7) TRAIN — matched simulator budget, fresh acquisition followed by dataset replay
...
# evaluation trajectories. Its stored importance weights and rows are shared by ALL models.

simulation_buffer = SimulationBuffer(CFG.simulation_budget)
posterior_buffers = {
    name: HistoricalPosteriorBuffer(CFG.historical_output_buffer_capacity, CFG.max_training_particles)
    for name in models
}
...
# Separate RNGs with common seeds align interpolation/mode choices without acquisition interference.
...
for step in range(1, TOTAL_TRAINING_STEPS + 1):
    ...
    if len(simulation_buffer) < CFG.simulation_budget:
        ...
        theta_target, x_batch, sample_weights = simulation_buffer.get_batch(
            epoch_indices[start:start + CFG.batch_size])
    ...
    for name, cfg in TRAIN_CONFIGS.items():
        incoming, info = make_training_prior_batch_np(
            cloud_rngs[name], mode_rngs[name], theta_target, x_batch,
            posterior_buffers[name], training_particles, cfg)
        ...
        if cfg.historical_output_prior_probability > 0.0:
            posterior_buffers[name].add_batch(x_batch, np.asarray(jax.device_get(posterior)))
    ...
    if step == 1 or step % CFG.log_every == 0 or step in (acquisition_steps, TOTAL_TRAINING_STEPS):
        print(f"step {step:6d}/{TOTAL_TRAINING_STEPS} | sims {len(simulation_buffer):,} | "
              f"replay {replay_epoch} | M {training_particles} | " + " | ".join(
                  f"{r['model']} ES {r['energy_score']:.5f}" for r in training_rows[-len(models):]))
...
for name in models:
    ...
    for ax, metric in zip(axes, ("energy_score", "grad_norm", "outside_prior_fraction")):
...
METHOD_LABELS = {
    ...
    "no_replay_xT": "No cloud replay: current x",
    "marginal_xT": "Buffer: predicted marginal + current x",
    "buffer_xT": "Original buffer: current x",
...
for scenario in SCENARIOS:
    ...
    print(f"\n{scenario.name} — energy-score gain versus original buffer/current-x:")
