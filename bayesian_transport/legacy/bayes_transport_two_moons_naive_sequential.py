...
This variant deliberately has no interpolated input clouds and no replay buffer.  It maintains one
particle cloud for the observed datum x_o.  At every optimizer step:

    1. theta targets are resampled from the current x_o cloud and passed through the simulator;
    2. that same current cloud is used as the prior particle set for every row in the batch;
...
for step in range(1, CFG.training_steps + 1):
    ...
    # parameters are drawn.  No exact-prior, interpolation, jitter, or replay branch exists here.
    ...
    # Every batch row gets exactly the same prior particles. copy() materializes writable,
    # contiguous storage before handing the batch to JAX.
    prior_batch = np.broadcast_to(
        sequential_prior_cloud[None, :, :],
        (CFG.batch_size, CFG.particles, 2),
    ).copy()
