...
The bottom of the file trains the same-condition NN/mean-class and GP baselines and both saves
...
class Config:
    ...
    # Optional structural location-scale wrapper around the parameterized transport cloud T_psi(x).
    # When EITHER structural component is active, use the empirical-zero-mean residual cloud
    #     Z_psi,j(x) = T_psi,j(x) - mean_k T_psi,k(x)
    # and compose
    #     Y_j(x) = mu_theta(x) + sigma_phi(x) * Z_psi,j(x).
    # theta (mean), phi (scale), and psi (transport) are all learnable and optimized jointly by the
    # same transport objective. No variance standardization is applied: only the empirical mean is removed.
    # mean_function_class:
    #   * "same_as_data" (DEFAULT): use the parametric class matching data_function.
    #   * any name in DATA_FUNCTIONS: deliberately specify/misspecify the structural mean class.
    #   * None: no additive structural mean; the historical transport is the second term directly.
    # scale_function_class:
    #   * "constant" (DEFAULT): learn one positive global sigma, initialized exactly at 1.
    #   * "same_as_mean", "same_as_data", or any DATA_FUNCTIONS name: positive x-dependent sigma
    ...
    likelihood_x_noise_std: float = 0.00000001      ## @TODO: 0.025
    ...
    # Attached nn.py comparison baseline. With mean_function_class=None this is the unchanged
    # historical StandardMLP. Otherwise it is constrained to the SAME resolved mean function class
    # and fitted by the same ordinary MSE loop, so the comparison does not regain a larger class.
...
def validate_config(cfg: Config) -> None:
    ...
    mean_class = cfg.mean_function_class
    if mean_class is not None and mean_class != "same_as_data" and mean_class not in DATA_FUNCTIONS:
        raise ValueError(
            "mean_function_class must be None, 'same_as_data', or one of DATA_FUNCTIONS."
        )
    scale_class = cfg.scale_function_class
    valid_scale_classes = {"constant", "same_as_mean", "same_as_data", *DATA_FUNCTIONS}
    if scale_class is not None and scale_class not in valid_scale_classes:
        raise ValueError(
            "scale_function_class must be None, 'constant', 'same_as_mean', 'same_as_data', "
            "or one of DATA_FUNCTIONS."
        )
    if scale_class == "same_as_mean" and mean_class is None:
        raise ValueError("scale_function_class='same_as_mean' requires a non-None mean_function_class.")
    if cfg.structural_scale_floor < 0.0 or cfg.structural_scale_floor >= 1.0:
        raise ValueError("structural_scale_floor must lie in [0,1).")
...
def resolve_mean_function_class(cfg: Config) -> str | None:
    """Resolve the configured structural mean class without changing the data generator."""
    if cfg.mean_function_class is None:
        return None
    if cfg.mean_function_class == "same_as_data":
        return str(cfg.data_function)
    return str(cfg.mean_function_class)


def resolve_scale_function_class(cfg: Config) -> str | None:
    """Resolve the positive structural scale class independently of the data generator."""
    if cfg.scale_function_class is None:
        return None
    if cfg.scale_function_class == "same_as_data":
        return str(cfg.data_function)
    if cfg.scale_function_class == "same_as_mean":
        mean_class = resolve_mean_function_class(cfg)
        if mean_class is None:
            raise ValueError("scale_function_class='same_as_mean' requires a mean function class.")
        return mean_class
    return str(cfg.scale_function_class)


def _structural_basis_scalar(x: Array, function_class: str) -> Array:
    """Fixed finite-dimensional basis for one scalar x; every DATA_FUNCTIONS class contains its truth.

    The nonlinear geometry/frequencies/change-points match the corresponding synthetic generator,
    while only global coefficients are learned. This deliberately restricts the learned location
    (and, if selected, log-scale) to the requested model class rather than letting coefficients vary
    freely with x. A leading constant basis is included in every class so zero/constant initialisation
    is well defined.
    """
    x = jnp.ravel(jnp.asarray(x))[0]
    one = jnp.asarray(1.0, dtype=x.dtype)

    if function_class == "constant":
        return jnp.stack([one])
    if function_class == "staircase_linear":
        return jnp.stack([one, jnp.floor(x), x])
    if function_class == "cosine_linear":
        return jnp.stack([one, jnp.cos(10.0 * x), x])
    if function_class == "square_wave":
        square = jnp.where(jnp.sin(9.0 * x) >= 0.0, one, -one)
        return jnp.stack([one, square, x])
    if function_class == "sawtooth":
        phase = jnp.mod(1.6 * x + 0.5, 1.0)
        return jnp.stack([one, phase - 0.5, x])
    if function_class == "triangle_wave":
        phase = jnp.mod(1.6 * x + 0.5, 1.0)
        triangle = 1.0 - 4.0 * jnp.abs(phase - 0.5)
        return jnp.stack([one, triangle, x])
    if function_class == "chirp":
        phase = 5.0 * x + 11.0 * x**2
        return jnp.stack([one, jnp.sin(phase)])
    if function_class == "frequency_jump":
        left = (x < 0.0).astype(x.dtype)
        right = 1.0 - left
        return jnp.stack([
            one, left * jnp.sin(4.0 * x), right * jnp.sin(22.0 * x)
        ])
    if function_class == "amplitude_jump":
        left = (x < 0.0).astype(x.dtype)
        right = 1.0 - left
        return jnp.stack([
            one, left * jnp.sin(9.0 * x), right * jnp.sin(9.0 * x)
        ])
    if function_class == "multiscale":
        return jnp.stack([one, jnp.sin(3.0 * x), jnp.sin(38.0 * x)])
    if function_class == "local_oscillation":
        envelope = jnp.exp(-0.5 * ((x - 0.45) / 0.20) ** 2)
        return jnp.stack([one, jnp.sin(3.5 * x), envelope * jnp.sin(42.0 * x)])
    if function_class == "spike_train":
        centres = (-1.05, -0.55, -0.05, 0.48, 0.92)
        spikes = [jnp.exp(-0.5 * ((x - c) / 0.045) ** 2) for c in centres]
        return jnp.stack([one, x, *spikes])
    if function_class == "cusp":
        dx = x - 0.1
        cusp = jnp.sign(dx) * jnp.sqrt(jnp.abs(dx))
        return jnp.stack([one, cusp, x])
    if function_class == "quantized_sine":
        quantized = jnp.round(4.0 * jnp.sin(6.0 * x)) / 4.0
        return jnp.stack([one, quantized, x])
    if function_class == "weierstrass":
        waves = [jnp.cos((3.0 ** k) * jnp.pi * x) for k in range(6)]
        return jnp.stack([one, *waves])
    if function_class == "piecewise_polynomial":
        left = (x < -0.55).astype(x.dtype)
        middle = ((x >= -0.55) & (x < 0.35)).astype(x.dtype)
        right = (x >= 0.35).astype(x.dtype)
        # The global intercept is the right-branch intercept; indicator offsets recover the others.
        return jnp.stack([
            one,
            left, left * x,
            middle, middle * x, middle * x**2,
            right * x,
        ])
    if function_class == "rational_cliff":
        dx = x - 0.22
        safe_dx = jnp.where(jnp.abs(dx) < 0.025, jnp.where(dx >= 0.0, 0.025, -0.025), dx)
        clipped_core = jnp.clip(0.18 / safe_dx, -1.8, 1.8)
        return jnp.stack([one, clipped_core, x])
    raise ValueError(f"Unknown structural function class {function_class!r}.")


def structural_design_matrix_np(x: np.ndarray, function_class: str) -> np.ndarray:
    """NumPy-facing design matrix for diagnostics/identifiability checks of the chosen class."""
    x = np.asarray(x, dtype=np.float32).reshape(-1)
    basis = jax.vmap(lambda xi: _structural_basis_scalar(xi, function_class))(jnp.asarray(x))
    return np.asarray(jax.device_get(basis), dtype=np.float64)


class PrespecifiedFunction(eqx.Module):
    """Global coefficients over a fixed structural basis; no x-dependent coefficient network."""
    coefficients: Array
    function_class: str = eqx.field(static=True)

    def __init__(self, function_class: str, *, initial_intercept: float = 0.0):
        self.function_class = str(function_class)
        n_basis = int(_structural_basis_scalar(jnp.asarray(0.0), self.function_class).shape[0])
        coefficients = jnp.zeros((n_basis,), dtype=jnp.float32)
        coefficients = coefficients.at[0].set(jnp.asarray(initial_intercept, dtype=jnp.float32))
        self.coefficients = coefficients

    def __call__(self, x: Array) -> Array:
        return jnp.dot(self.coefficients, _structural_basis_scalar(x, self.function_class))
...
# Finite-sample identifiability diagnostic for the selected structural mean basis. Full column rank
# means the global coefficients are uniquely determined by noiseless values at these training x's.
STRUCTURAL_MEAN_CLASS = resolve_mean_function_class(CFG)
if STRUCTURAL_MEAN_CLASS is None:
    structural_mean_design_rank = None
    structural_mean_design_dim = None
    structural_mean_design_condition = None
else:
    _phi_train = structural_design_matrix_np(X_train, STRUCTURAL_MEAN_CLASS)
    structural_mean_design_rank = int(np.linalg.matrix_rank(_phi_train))
    structural_mean_design_dim = int(_phi_train.shape[1])
    _svals = np.linalg.svd(_phi_train, compute_uv=False)
    structural_mean_design_condition = (
        float(np.inf) if _svals[-1] <= np.finfo(np.float64).eps
        else float(_svals[0] / _svals[-1])
    )
...
if STRUCTURAL_MEAN_CLASS is not None:
    print(
        f"Structural mean design on X_train: rank={structural_mean_design_rank}/"
        f"{structural_mean_design_dim} | condition={structural_mean_design_condition:.3e}"
    )
print(
    "Structural output: "
    f"mean_class={resolve_mean_function_class(CFG)!r} "
    f"(configured={CFG.mean_function_class!r}) | "
    f"scale_class={resolve_scale_function_class(CFG)!r} "
    f"(configured={CFG.scale_function_class!r}) | "
    + (
        "Y_j=mu_theta(x)+sigma_phi(x)*[T_psi,j(x)-mean_k T_psi,k(x)]; theta/phi/psi jointly learned."
        if (CFG.mean_function_class is not None or CFG.scale_function_class is not None)
        else "Historical cloud-only output Y_j=T_psi,j (no structural centering)."
    )
)
if CFG.mean_function_class is None and CFG.scale_function_class is None:
    print("Structural wrapper is fully disabled: output equals the historical transport cloud exactly.")
...
class ConditionalParticleTransport(eqx.Module):
    ...
    mean_function: PrespecifiedFunction | None
    scale_function: PrespecifiedFunction | None
    ...
    mean_function_class: str | None = eqx.field(static=True)
    scale_function_class: str | None = eqx.field(static=True)
    structural_scale_floor: float = eqx.field(static=True)
    ...
    def __init__(self, cfg: Config, scaling: Scaling, *, key: Array):
        ...
        # Structural location-scale wrapper. theta (mean coefficients), phi (scale coefficients),
        # and psi (all historical transport parameters) are optimized jointly by the same objective.
        # These deterministic initialisations make mu(x)=0 and sigma(x)=1 at step zero. If either
        # structural component is active, compose_structural_output additionally mean-centres T_psi;
        # if BOTH are None, the historical transport output is returned exactly with no centring.
        self.mean_function_class = resolve_mean_function_class(cfg)
        self.scale_function_class = resolve_scale_function_class(cfg)
        self.structural_scale_floor = float(cfg.structural_scale_floor)
        self.mean_function = (
            None if self.mean_function_class is None
            else PrespecifiedFunction(self.mean_function_class, initial_intercept=0.0)
        )
        if self.scale_function_class is None:
            self.scale_function = None
        else:
            raw_one = _inverse_softplus_scalar(1.0 - self.structural_scale_floor)
            self.scale_function = PrespecifiedFunction(
                self.scale_function_class, initial_intercept=raw_one
            )

    def structural_mean(self, x: Array) -> Array:
        """Configured global mean-class output; None means an exact additive zero."""
        if self.mean_function is None:
            return jnp.asarray(0.0)
        return self.mean_function(x)

    def structural_scale(self, x: Array) -> Array:
        """Positive scale output; None means exact multiplication by one."""
        if self.scale_function is None:
            return jnp.asarray(1.0)
        return self.structural_scale_floor + jax.nn.softplus(self.scale_function(x))

    def compose_structural_output(self, transported_samples: Array, x: Array) -> Array:
        """Compose jointly learned mean/scale/transport components into the final particle cloud.

        Let T_psi,j(x) denote the existing Transformer/particle-transport output, with psi containing
        all of its learnable parameters. When either structural mean mu_theta or structural scale
        sigma_phi is enabled, the residual cloud is

            Z_psi,j(x) = T_psi,j(x) - mean_k T_psi,k(x),

        so its empirical particle mean is exactly zero. The final samples are then

            Y_j(x) = mu_theta(x) + sigma_phi(x) * Z_psi,j(x).

        theta, phi, and psi are differentiated and optimized jointly by the existing objective.
        No variance normalization is performed. When BOTH structural classes are None, this method
        returns T_psi,j(x) exactly, preserving the historical cloud-only model.
        """
        structural_active = (
            self.mean_function_class is not None or self.scale_function_class is not None
        )
        residual_samples = (
            transported_samples - jnp.mean(transported_samples)       ## @TODO
            # transported_samples
            if structural_active
            else transported_samples
        )
        return self.structural_mean(x) + self.structural_scale(x) * residual_samples
...
def _transport_objective(
    ...
    if cfg.likelihood_conditioning_mode == "direct":
        transported = jax.vmap(
            lambda p, x, k: model.direct_transport(
                p, x, key=k, inference=False
            )
        )(prior, x_observations, row_keys)
        posterior = jax.vmap(model.compose_structural_output)(transported, x_observations)
        posterior_by_prefix = posterior[:, None, :]
        prefix_counts = jnp.asarray([1], dtype=jnp.int32)
    else:
        transported_by_prefix, prefix_counts = jax.vmap(
            lambda p, obs, k: model.predict_prefixes(
                p, obs, key=k, inference=False
            )
        )(prior, x_observations, row_keys)
        # Each Transformer prefix contains repeated independent noisy observations around the same
        # clean x_pair. Use their prefix mean as the scalar coordinate for the structural mu/sigma.
        cumulative_x = jnp.cumsum(x_observations, axis=1)
        prefix_x = cumulative_x[:, prefix_counts - 1] / prefix_counts[None, :]
        posterior_by_prefix = jax.vmap(
            lambda clouds, xs: jax.vmap(model.compose_structural_output)(clouds, xs)
        )(transported_by_prefix, prefix_x)
...
def make_nn_comparison_model(cfg: Config, *, key: Array) -> eqx.Module:
    """Use the same structural mean class as transport; None preserves the historical StandardMLP."""
    mean_class = resolve_mean_function_class(cfg)
    if mean_class is None:
        return StandardMLP(cfg, key=key)
    # A generic x-dependent MLP would leave the requested finite-dimensional function class.
    # The fair comparison therefore learns the SAME global basis coefficients under ordinary MSE.
    return PrespecifiedFunction(mean_class, initial_intercept=0.0)


def nn_comparison_label(cfg: Config) -> str:
    mean_class = resolve_mean_function_class(cfg)
    return "Standard MLP" if mean_class is None else f"Mean baseline ({mean_class} class)"
...
def train_mlp(
    ...
    *,
    ...
    if model is None:
        model = make_nn_comparison_model(cfg, key=jax.random.key(cfg.seed))
    ...
    def loss_fn(m, xb, yb):
        pred = jnp.reshape(jax.vmap(m)(xb), (-1,))
        target = jnp.reshape(yb, (-1,))
    ...
    for epoch in range(cfg.nn_epochs):
        ...
        if epoch % 500 == 0 or epoch == cfg.nn_epochs - 1:
            print(f"NN/mean baseline epoch {epoch:4d}/{cfg.nn_epochs} | MSE {float(loss):.6f}")
    return model


def predict_mlp(model: eqx.Module, x: np.ndarray) -> np.ndarray:
...
def print_model_parameter_counts(
    ...
    mean_params = 0 if transport.mean_function is None else count_eqx_parameters(transport.mean_function)
    scale_params = 0 if transport.scale_function is None else count_eqx_parameters(transport.scale_function)
    print(
    ...
    print(f"  Structural mean ({transport.mean_function_class}): {mean_params:,}")
    print(f"  Structural scale ({transport.scale_function_class}): {scale_params:,}")
    print(
        f"  Parameterized posterior transport T_psi: "
        f"{transport_total - likelihood_params - direct_x_params - mean_params - scale_params:,}"
    )
    mlp_label = (
        "Standard MLP" if isinstance(mlp, StandardMLP)
        else f"Mean baseline ({getattr(mlp, 'function_class', '?')} class)"
    )
    print(f"{mlp_label}: {count_eqx_parameters(mlp):,} trainable scalar parameters")
    print(
...
nn_init = make_nn_comparison_model(CFG, key=jax.random.key(CFG.seed))
gp_init = make_gp(CFG)
...
print(f"\n--- training {nn_comparison_label(CFG)} ONCE ---")
...
def _predict_batch(
    ...
    if model.likelihood_conditioning_mode == "direct":
        ...
        return jax.vmap(model.compose_structural_output)(transported, x_observations)
    transported = jax.vmap(
        lambda p, obs: model(p, obs, observation_count, inference=True)
    )(prior, x_observations)
    # Deployment observations are clean repeated copies of x, so this mean is exactly the query x.
    structural_x = jnp.mean(x_observations, axis=1)
    return jax.vmap(model.compose_structural_output)(transported, structural_x)
...
def plot_comparison(
    ...
    axes[0].set_title(nn_comparison_label(cfg))
    ...
    *,
    ...
    metrics = {
        ...
        **baseline_metrics,
        ...
        "structural_output": {
            "mean_function_class": transport.mean_function_class,
            "scale_function_class": transport.scale_function_class,
            "mean_coefficients": (
                None if transport.mean_function is None
                else np.asarray(jax.device_get(transport.mean_function.coefficients)).tolist()
            ),
            "scale_raw_coefficients": (
                None if transport.scale_function is None
                else np.asarray(jax.device_get(transport.scale_function.coefficients)).tolist()
            ),
            "mean_design_rank": structural_mean_design_rank,
            "mean_design_dimension": structural_mean_design_dim,
            "mean_design_condition": structural_mean_design_condition,
            "transport_samples_recentered": bool(
                transport.mean_function_class is not None or transport.scale_function_class is not None
            ),
            "transport_parameterization": "T_psi; jointly optimized with mu_theta and sigma_phi",
        },
...
def _plot_nn_axis(ax) -> None:
    ...
    ax.set_title(nn_comparison_label(CFG))
    ax.legend(loc="best", fontsize=9)
...
def evaluate_and_plot_prior_modes(
    ...
    *,
    ...
    This function performs inference and plotting only. The NN/mean-class baseline, Gaussian Process, and
