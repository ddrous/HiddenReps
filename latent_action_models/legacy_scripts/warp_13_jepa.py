...
CONFIG = {
    ...
    "nb_epochs": 150,
    "print_every": 1,
    "batch_size": 3 if SINGLE_BATCH else 32,
    "learning_rate": 1e-4,
    "p_forcing": 0.5,
    ...
    "ae_loss_type": "encdec",  # Choose between "encdec", "decenc", or None
    "ae_loss_weight": 1.0,     # Weight of the autoencoding loss relative to latent dynamics loss
...
class WARP(eqx.Module):
    ...
    def _get_preds_single(self, ref_video, p_forcing, key, inf_context_ratio):
        ...
        def scan_step(z_prev, scan_inputs):
            ...
            # Return z_next as the scan state, and BOTH z_next and pred_out to the sequence
    ...
    def __call__(self, ref_videos, p_forcing, keys, coords_grid, inf_context_ratio, precompute_ref_diffs=False):
        ...
        pred_latents, pred_videos = batched_fn(ref_videos, p_forcing, keys, inf_context_ratio)
...
if TRAIN:
    ...
    def train_step(model, opt_state, keys, ref_videos, coords_grid, p_forcing):
        def loss_fn(m):
            ...
            # Forward pass: Extract both predicted latents and decoded pixels
            pred_latents, pred_videos = m(ref_videos, p_forcing, keys, coords_grid, 0.0, precompute_ref_diffs=False)

            # Transpose reference videos to (B, T, C, H, W) for encoder
            ref_videos_enc = jnp.transpose(ref_videos, (0, 1, 4, 2, 3))

            # Encode ground truth to target latents
            target_latents = jax.vmap(jax.vmap(m.encoder))(ref_videos_enc)

            # Align targets: scan predicts t=1..T. Shift target latents accordingly.
            target_latents_shifted = jnp.concatenate([target_latents[:, 1:], target_latents[:, -1:]], axis=1)

            # --- 1. LATENT DYNAMICS LOSS (Primary) ---
            latent_loss = jnp.mean((pred_latents - target_latents_shifted)**2)

            # --- 2. AUTOENCODING LOSS (Auxiliary) ---
            ae_loss = 0.0
            if CONFIG["ae_loss_type"] == "encdec":
                # Encode -> Decode -> Match GT Pixels
                decoded_pixels = jax.vmap(jax.vmap(m.decoder))(target_latents)
                if CONFIG["use_nll_loss"]:
                    decoded_mean = decoded_pixels[:, :, :C]
                    ae_loss = jnp.mean((decoded_mean - ref_videos_enc)**2)
                else:
                    ae_loss = jnp.mean((decoded_pixels - ref_videos_enc)**2)

            elif CONFIG["ae_loss_type"] == "decenc":
                # Pred Latent -> Decode -> Re-Encode -> Match Pred Latent
                decoded_pixels = jax.vmap(jax.vmap(m.decoder))(pred_latents)
                if CONFIG["use_nll_loss"]:
                    decoded_mean = decoded_pixels[:, :, :C]
                else:
                    decoded_mean = decoded_pixels
                
                re_encoded_latents = jax.vmap(jax.vmap(m.encoder))(decoded_mean)
                ae_loss = jnp.mean((re_encoded_latents - jax.lax.stop_gradient(pred_latents))**2)
