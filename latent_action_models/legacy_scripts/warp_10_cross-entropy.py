...
CONFIG = {
    "seed": 2026,
    "nb_epochs": 10000,
    "batch_size": 8 if not SINGLE_BATCH else 1,
    "learning_rate": 1e-6,
    "print_every": 500,
    "p_forcing": 0.0,
...
def numpy_collate(batch):
    ...
    # --- CHANGED: Do NOT divide by 255.0 to maintain 256 pixel classes ---
...
def sbimshow(img, title="", ax=None):
    # --- CHANGED: Normalize to [0, 1] purely for Matplotlib rendering ---
...
class WARP(eqx.Module):
    ...
    def __init__(self, root_width, root_depth, num_freqs, frame_shape, key):
        ...
        # --- CHANGED: Output 256 dimensions for CE class logits ---
        ...
        # print(f"Initialized theta_base with shape: {self.theta_base.shape}")

        self.hypernet_phi = CNNEncoder(in_channels=C, out_dim=self.d_theta*1, spatial_shape=(H, W), key=k_phi, hidden_width=16, depth=2)
        ...
        self.A = jax.random.normal(k_A, (self.d_theta*1, self.d_theta*1)) * 0
        # self.A = jnp.zeros((self.d_theta*1,))
        self.B = jnp.zeros((self.d_theta*1, CONFIG["rec_feat_dim"]))

        print(f"Initialized WARP with d_theta={self.d_theta}, A shape={self.A.shape}, B shape={self.B.shape}")
    ...
    def render_pixels(self, thetas, coords):
        def render_pt(theta, coord):
            ...
            # --- CHANGED: Return logits directly ---
            return out
    ...
    def _get_thetas_and_preds_single(self, ref_video, p_forcing, key, coords_grid, inf_context_ratio):
        ...
        # theta_mu, theta_scale = jnp.split(theta_0, 2, axis=-1)
        ...
        def scan_step(state, scan_inputs):
            ...
            # --- CHANGED: Calculate Expected Value for the predicted pixels ---
            ...
            # Return tuple of (logits, expected_pixels)
...
if TRAIN:
    ...
    def train_step(model, opt_state, keys, ref_videos, coords_grid, p_forcing):
        def loss_fn(m):
            ...
            # 1. Standard Forward Pass returns (Logits, Pixels)
            pred_logits, pred_pixels = m(ref_videos, p_forcing, keys, coords_grid, 0.0)
            ...
            # Squeeze channel dimension to match categorical expectations
            ...
            logits_flat_rand = eqx.filter_vmap(m.render_pixels, in_axes=(0, None))(thetas_frame_rand, coords_grid.reshape(-1, 2))
            
            logits_frame_rand = logits_flat_rand.reshape(B, N_init, H, W, 256)
            labels_rand = rand_gt_frames.astype(jnp.int32).squeeze(-1)

            loss_ae = jnp.mean(optax.softmax_cross_entropy_with_integer_labels(
                logits=logits_frame_rand, 
                labels=labels_rand
            ))
    ...
    for epoch in range(CONFIG["nb_epochs"]):
        ...
        if (epoch+1) % (CONFIG["nb_epochs"]//10)==0:
            ...
            # Extract the expected pixels (index 1) for visualization
...
val_keys = jax.random.split(key, CONFIG["batch_size"])
_, final_pixels = evaluate(model, sample_batch, 0.0, val_keys, coords_grid, 0.0)
