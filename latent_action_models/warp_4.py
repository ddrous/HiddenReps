...
CONFIG = {
    ...
    "rec_feat_dim": 64,
    "latent_dim": 128,        # NEW: Size of the hidden space for recurrence
    "root_width": 32,
    "root_depth": 2,
    "num_fourier_freqs": 6
}
...
class WARP(eqx.Module):
    ...
    def __init__(self, root_width, root_depth, num_freqs, frame_shape, latent_dim, key):
        k_root, k_A, k_B, k_phi, k_psi, k_enc, k_dec = jax.random.split(key, 7)
        ...
        # --- NEW: MLPs to project between Weight Space and Latent Recurrence Space ---
        self.encoder = eqx.nn.MLP(in_size=self.d_theta*2, out_size=self.latent_dim, width_size=256, depth=2, key=k_enc)
        self.decoder = eqx.nn.MLP(in_size=self.latent_dim, out_size=self.d_theta*2, width_size=256, depth=2, key=k_dec)

        # --- MODIFIED: Recurrence happens strictly in Latent Space ---
        self.A = jnp.eye(self.latent_dim)
        self.B = jnp.zeros((self.latent_dim, CONFIG["rec_feat_dim"]))
    ...
    def _get_thetas_and_preds_single(self, ref_video, p_forcing, key, coords_grid, inf_context_ratio):
        ...
        # Hypernetwork defines the initialization in weight space
        theta_0 = self.hypernet_phi(init_gt_frame_chw)
        
        # Encode initialization down into the latent recurrence space
        z_0 = self.encoder(theta_0)

        def scan_step(state, scan_inputs):
            gt_curr_frame, step_idx = scan_inputs
            z, prev_frame_selected, k = state
            k, subk = jax.random.split(k)

            # 1. Decode current latent state back to weight space
            theta_decoded = self.decoder(z)

            # 2. Split and apply the FiLM projection for rendering
...
if TRAIN:
    ...
    def train_step(model, opt_state, keys, ref_videos, coords_grid, p_forcing):
        def loss_fn(m):
            ...
            # 1. Hypernetwork to get weight-space initialization
            theta_rand = eqx.filter_vmap(m.hypernet_phi)(flat_gt_frames_chw) 

            # 2. Encode to Latent Space, then Decode back to Weight Space
            z_rand = eqx.filter_vmap(m.encoder)(theta_rand)
            theta_rand_decoded = eqx.filter_vmap(m.decoder)(z_rand)

            # 3. Compute FiLM rendering
            theta_mu, theta_scale = jnp.split(theta_rand_decoded, 2, axis=-1)
            theta_rand_scaled = m.theta_base*(1+theta_scale) + theta_mu

            thetas_frame_rand = jnp.tile(theta_rand_scaled[:, None, :], (1, H * W, 1))
            ...
            # Explicit Autoencoder Weight Reconstruction (Crucial for stabilizing the latent space mappings)
            loss_weights = jnp.mean((theta_rand_decoded - theta_rand)**2)

            # return loss_full + 1.0 * loss_ae + 1.0 * loss_weights
            return loss_full + 1.0 * loss_ae
