...
CONFIG = {
    ...
    # Model & DataHyperparameters
    "taylor_radius": 0.5,
    "context_dim": 1,
    "width_size": 32,
    "noise_std": 0.015,
    "data_samples": 1000,
    "train_segments": [2, 3, 4, 5, 6],
    "segments": 9,
    "x_range": [-1.5, 1.5],

    # PAM Hyperparameters
    "pam_outer_steps_max": 100,
...
# Generate Data
SEED = CONFIG["seed"]
TRAIN_SEG_IDS = CONFIG["train_segments"]

data, segs = gen_data(SEED, 
                      CONFIG["data_samples"], 
...
class EBMNet(eqx.Module):
    ...
    # Static attributes for reconstructing the target network
    ...
    def __init__(self, key, context_dim, y_mean=0.0):
        ...
        # 1. Define the Target Network Template (takes x, y -> energy)
        # Input size 2 (x and y)
        target_template = eqx.nn.MLP(in_size=2, out_size=1, 
                                     width_size=width_size, depth=3, 
                                     activation=jax.nn.relu, key=k1)
        
        # 2. Flatten to get shapes and size
        ...
        # Save static structure of target
        _, self.target_static = eqx.partition(target_template, eqx.is_array)

        # 3. Define Hypernetwork: Context -> Target Weights
        self.hypernet = eqx.nn.MLP(in_size=context_dim, out_size=target_param_size,
                                   width_size=width_size * 2, depth=3, # Slightly larger for hypernet
                                   activation=jax.nn.relu, key=k2)

    def _get_target_net(self, flat_params):
        # Reconstitute the target network from flattened parameters
    ...
    def _solve_training(self, x_batch, c_batch):
        # 1. Reconstitute Neural Nets for the batch
        # hypernet outputs: (Batch, ParamSize)
        flat_params_batch = jax.vmap(self.hypernet)(c_batch)
        
        # We need to optimize y for each sample. 
        # The target network is different for each sample (weights depend on c).
        ...
        def step(state, _):
            ...
            def energy_fn(y, x, flat_params):
                model = self._get_target_net(flat_params)
                inp = jnp.concatenate([x, y], axis=0)
                return jnp.squeeze(model(inp))
            
            # Gradients: vmap over batch of x, y, and specific weights
            grads = jax.vmap(jax.grad(energy_fn))(y_curr, x_batch, flat_params_batch)
    ...
    def _solve_inference(self, x_batch):
        # Inference: Optimize Y and C (init 0)
        # Note: In Hypernet formulation, calculating grad w.r.t C involves 
        # backprop through the Hypernet reconstruction.
        ...
        def step(state, _):
            ...
            def energy_c(y, c, x):
                # Generate weights from C on the fly
                flat_params = self.hypernet(c)
                model = self._get_target_net(flat_params)
                inp = jnp.concatenate([x, y], axis=0)
                return jnp.squeeze(model(inp))
...
class TaylorEBMNet(EBMNet):
    def _solve_training(self, x_batch, c_batch):
        # BATCHED Training Loop with Taylor Logic on Weights
        ...
        # 2. Compute Taylor Approximation of Weights: theta(c) ~ theta(c0) + J(c0)(c-c0)
        def get_taylor_weights(c, c0):
            # Value and Jacobian-Vector Product
            theta_0, theta_jvp = jax.jvp(self.hypernet, (c0,), (c - c0,))
            return theta_0 + theta_jvp

        # Compute weights for the whole batch
        flat_params_batch = jax.vmap(get_taylor_weights)(c_batch, c0_batch)

        # 3. Optimize Y using these Taylor-approximated weights
        y_init = jnp.full((x_batch.shape[0], 1), self.y_mean)
        opt = optax.adam(inner_gd_lr)
        opt_state = opt.init(y_init)
