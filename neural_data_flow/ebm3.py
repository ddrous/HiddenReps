...
class LinearModel(eqx.Module):
    ...
    def __call__(self, x, c=None, key=None):
        # Training forward: returns pred, aux
        return jax.vmap(self.layer)(x), None 
    def predict(self, x):
        # Inference forward: returns pred only
        pred, _ = self(x)
        return pred
...
class TaylorMLP(eqx.Module):
    ...
    def _forward(self, x):
        ...
        return x
    def __call__(self, x, c=None, key=None):
        # Training logic (Taylor augmentation)
...
class BaseEBM(eqx.Module):
    ...
    def _solve_y(self, x):
        # Solves for y given x (Training/Inference same for BaseEBM)
...
class EBMNet(eqx.Module):
    ...
    def __init__(self, key, context_dim, y_mean=0.0):
        self.net = eqx.nn.MLP(in_size=2 + context_dim, out_size=1, 
                              width_size=width_size, depth=3, 
                              activation=ACTIVATION, key=key)
        self.y_mean = y_mean
    ...
    def __call__(self, x, c):
        # Training forward: Solve for Y given C
        return self._solve_training(x, c)

    def _solve_training(self, x_batch, c_batch):
        # BATCHED Training Loop: Fix C, Optimize Y
        ...
        def step(state, _):
            ...
            grads = jax.vmap(jax.grad(self.energy, argnums=1))(x_batch, y_curr, c_batch)
            updates, opt_state = opt.update(grads, opt_state, y_curr)
            y_next = optax.apply_updates(y_curr, updates)
            return (y_next, opt_state), None
    ...
    def predict(self, x_batch):
        # Inference forward: Solve for Y and C
        return self._solve_inference(x_batch)

    def _solve_inference(self, x_batch):
        # BATCHED Inference Loop: Optimize Y and C (init 0)
        B = x_batch.shape[0]
        ...
        def step(state, _):
            ...
            grads = jax.vmap(jax.grad(self.energy, argnums=(1, 2)))(x_batch, y_curr, c_curr)
...
class TaylorEBMNet(EBMNet):
    def energy_taylor(self, x, y, c, c0):
        # Single Sample Taylor Energy
        def base_energy(c_arg): return self.energy(x, y, c_arg)
        E_c0, dE_dc0 = eqx.filter_jvp(base_energy, (c0,), (c - c0,))
        return E_c0 + dE_dc0

    def _solve_training(self, x_batch, c_batch):
        # BATCHED Training Loop with Taylor Logic
        # 1. Compute c0 (Nearest Neighbor in Batch)
        #    Note: dists matrix (B, B)
        dists = jnp.linalg.norm(c_batch[:, None, :] - c_batch[None, :, :], axis=-1)
        #    Add infinity to diagonal to avoid self-selection
        dists = dists + jnp.eye(dists.shape[0]) * 1e9
        nearest_idx = jnp.argmin(dists, axis=1)
        c0_batch = c_batch[nearest_idx]

        # 2. Optimize Y using Taylor Energy around c0
        y_init = jnp.full((x_batch.shape[0], 1), self.y_mean)
        opt = optax.adam(inner_gd_lr)
        opt_state = opt.init(y_init)
        ...
        def step(state, _):
            ...
            # Gradient of Taylor Energy w.r.t Y (arg 0 of lambda)
            # vmap inputs: (x, y, c, c0) -> grad per sample
            def taylor_loss_grad(x_i, y_i, c_i, c0_i):
                return jax.grad(lambda y_arg: self.energy_taylor(x_i, y_arg, c_i, c0_i))(y_i)
            
            grads = jax.vmap(taylor_loss_grad)(x_batch, y_curr, c_batch, c0_batch)
...
class ContextEBM(eqx.Module):
    ...
    def __init__(self, key, num_train_samples, y_mean=0.0, context_dim=1):
        self.nn = EBMNet(key, context_dim, y_mean)
        self.ctx = Contexts(num_train_samples, context_dim)
    def __call__(self, x, c):
        return self.nn(x, c) # Delegates to nn._solve_training via __call__
    def predict(self, x):
...
class TaylorEBM(eqx.Module):
    ...
    ctx: Contexts
    def __init__(self, key, num_train_samples, y_mean=0.0, context_dim=1):
        self.nn = TaylorEBMNet(key, context_dim, y_mean)
        self.ctx = Contexts(num_train_samples, context_dim)
    def __call__(self, x, c): 
        return self.nn(x, c) # Delegates to nn._solve_training via __call__
    def predict(self, x):
...
if TRAIN:
    ...
    for name, model in models_config:
        ...
        is_pam_model = name in ["ContextEBM", "TaylorEBM"]
        ...
        if not is_pam_model:
            ...
            def compute_loss(model, x, y, key):
                pred, _ = model(x, key=key) # __call__ used for training
            ...
            @eqx.filter_jit
            def train_step_nn(nn, nn_old, ctx, opt_state, x, y, indices):
                
                def prox_loss_nn(nn_params, ctx_static):
                    c_batch = ctx_static(indices)
                    # nn_params(...) Calls __call__, which calls _solve_training
                    y_pred = nn_params(x, c_batch) 
            ...
            @eqx.filter_jit
            def train_step_ctx(ctx, ctx_old, nn, opt_state, x, y, indices):
                
                def prox_loss_ctx(ctx_params, nn_static):
                    c_batch = ctx_params(indices)
                    # nn_static(...) Calls __call__, which calls _solve_training
                    y_pred = nn_static(x, c_batch)
                    task_loss = jnp.mean((y_pred - y) ** 2)
            ...
            for out_step in range(CONFIG["pam_outer_steps_max"]):
                ...
                for in_step_nn in range(CONFIG["pam_inner_steps_nn"]):
                    ...
                    for bx, by, b_idx in dm.get_iterator():
                        nn_curr, opt_state_nn, loss, diff = train_step_nn(nn_curr, nn_old, model.ctx, opt_state_nn, bx, by, b_idx)
                        batch_losses.append(loss)
                    
                    losses_nn_history.append(np.mean(batch_losses))

                    # diff_rel = params_diff_norm_squared(nn_curr, nn_old) / (params_norm_squared(nn_old) + 1e-9)
                    # if diff_rel < CONFIG["pam_tol_nn"] and out_step > 0:
                    #     print(f"    -> Breaking Inner NN at step {in_step_nn} (diff {diff_rel:.2e})")
                    #     break
                
                # Update main model with new NN
                ...
                for in_step_ctx in range(CONFIG["pam_inner_steps_ctx"]):
                    batch_losses_ctx = []
                    for bx, by, b_idx in dm.get_iterator():
                        ctx_curr, opt_state_ctx, loss, diff = train_step_ctx(ctx_curr, ctx_old, model.nn, opt_state_ctx, bx, by, b_idx)
                        batch_losses_ctx.append(loss)
                    
                    losses_ctx_history.append(np.mean(batch_losses_ctx))

                    # diff_rel = params_diff_norm_squared(ctx_curr, ctx_old) / (params_norm_squared(ctx_old) + 1e-9)
                    # if diff_rel < CONFIG["pam_tol_ctx"] and out_step > 0:
                    #     print(f"    -> Breaking Inner Ctx at step {in_step_ctx} (diff {diff_rel:.2e})")
                    #     break
                
                # Update main model with new Contexts
...
for i, (name, model, _) in enumerate(trained_models):
    ...
    plt.plot(x_grid, y_grid, color=colors[i % len(colors)], linewidth=2.5, label=f"{name} (MSE: {mse:.4f})")
