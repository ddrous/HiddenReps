...
CONFIG = {
    ...
    "lr_nn": 0.01,    # Standard Models & PAM NN
...
class EBMNet(eqx.Module):
    ...
    def _solve_training(self, x_batch, c_batch, key=None):
        # Reconstitute Neural Nets for the batch
...
class TaylorEBMNet(EBMNet):
    ...
    def __init__(self, key, context_dim, y_mean=0.0, radius=0.2):
        super().__init__(key, context_dim, y_mean)
        self.radius = radius

    def _solve_training(self, x_batch, c_batch, key):
        # DOUBLE TAYLOR EXPANSION:
        # 1. Weights wrt Context: theta(c) ~ theta(c0) + ...
        # 2. Energy wrt Input: E(x) ~ E(x0) + ...
        
        if key is None:
            # Fallback if key is missing (though should be provided)
            # We create a deterministic key to avoid crashes, but warn conceptually
        ...
        # --- B. Input Expansion Setup ---
        epsilon = jax.random.uniform(key, x_batch.shape, minval=-self.radius, maxval=self.radius)
        ...
        def step(state, _):
            ...
            # Loss function: Taylor Approximation of Energy w.r.t x around x0
            def energy_taylor_x(y, x, x0, flat_params):
                model = self._get_target_net(flat_params)
                
                # Function to approximate: E(x_arg, y)
                def energy_wrt_x(x_arg):
                    inp = jnp.concatenate([x_arg, y], axis=0)
                    return jnp.squeeze(model(inp))
                
                # Compute E(x0) + JVP(x-x0)
                E_val, E_jvp = jax.jvp(energy_wrt_x, (x0,), (x - x0,))
                return E_val + E_jvp
            
                # ## If no Taylor expansion w.r.t x:
                # return jnp.squeeze(model(jnp.concatenate([x, y], axis=0)))

            # Compute grads of this approximated energy w.r.t Y
            grads = jax.vmap(jax.grad(energy_taylor_x, argnums=0))(
                y_curr, x_batch, x0_batch, flat_params_batch
            )
...
class TaylorEBM(eqx.Module):
    ...
    def __init__(self, key, num_train_samples, y_mean=0.0, context_dim=1):
        k1, k2 = jax.random.split(key)
        self.nn = TaylorEBMNet(k1, context_dim, y_mean, radius=CONFIG["taylor_radius"])
        self.ctx = Contexts(k2, num_train_samples, context_dim, CONFIG["context_init"])
    def __call__(self, x, c, key=None): 
        return self.nn(x, c, key)
    def predict(self, x):
